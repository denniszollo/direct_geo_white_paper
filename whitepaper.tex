...
In order to compare and analyze results from Pix4D, a total of 12 variations of settings and data were selected for rendering. 
...
The negative effect of overlap and sidelap is clearly shown in diagram ?? . The difference of RMS error from configuration 11 to 8 (Ublox) is quiet significant. In average, every other line renders from Ublox data resulted in 3 times more RMS error than Piksi. If one is not using a Piksi, the only way to bring down this error is to use GCPs. This observation proves 2 very important arguments. Firstly, this confirms that using a Piksi, an ideal surveying mission can be conducted with 25-23 percent overlap and 30 percent sidelap compared to the standard 75 and 60 percent respectively. This allows the user to get more out of their system. One can cover a bigger area with their machine, finish their job quicker and increase their ground sampling distance by flying at a lower altitude. 
